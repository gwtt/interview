...
> 2. -Xmx：设置堆内存的最大大小。 
> 3. -Xmn：设置新生代的大小。
> 4.  -XX:NewRatio：设置新生代与老生代的大小比例。
> 5.  -XX:MaxNewSize：设置新生代的最大大小。
> 6.  -XX:SurvivorRatio：设置Eden区和Survivor区的大小比例。
> 7. -XX:PermSize、-XX:MaxPermSize：用于设置永久代的初始大小和最大大小（在 JDK8 及之后版本，永久代已被元空间替代）。
>
> 实际项目中，我在elasticsearch里面配置过Xmx的大小，在自己项目中配置过ZGC垃圾回收器

### 27. 你在项⽬中遇到过 GC 问题吗？怎么分析和解决的？

> 遇到过一次，主要是健康打卡这个项目，full gc频繁，内存过几天就会爆满。后来运维同学说线程数量异常，我们去找对应的线程池，发现有个线程池使用默认策略，没有配置。
...
> 3.优化代码逻辑
>
> 4.使用G1收集器，将堆内存分为多个区域来优化收集时间和停顿时间，降低Full GC频率

### 29. 项⽬中实践过 JVM 调优吗？怎么做的？

> 调优不多，将堆内存设置大一点，GC日志打印出来，选择G1垃圾收集器。